        ...
        <div className={scss.content}>
          ...
          <div className={scss.divGlobal}>
            <div className={scss.divLeft}>
              ...
              <div className={scss.divBackgroundLeft}>
                ...
                {reduceThree ? (
                  <div>
                    <div className={scss.colourDiv} onClick={() => setReduceThree(false)}>
                      <p>Цвет</p>
                  ...
                  <div>
                    {hideColours ? (
                      ...
                      <div>
                        ...
                        <div className={scss.clapDiv} onClick={() => setHideColours(true)}>
            ...
            <div className={scss.divRight}>
                ...
                <div className={scss.categoriesRight}>
                  {categoryArray.map((categories) => (
                    <div className={scss.category_right}>
                      <p>{categories}</p>
                      <IconX onClick={() => handleSelectedCategory(categories)} />
                    </div>
                  ))}
                </div>
              ...
              <div className={scss.cardss}>
                {posts && posts
                  .filter((post) => {
                    ...
                    const isMatchingCategory = post.name && selectedCategories.some((category) => post.name.includes(category))
                    return isMatchingCategory
                  })
                  .filter((post) => {
                    const postPrice = post.price ? parseFloat(post.price.replace(' с', '').replace(' ', '')) : null
                    const lowPrice = priceLow !== "" ? parseFloat(priceLow) : null
                    const highPrice = priceHigh !== "" ? parseFloat(priceHigh) : null

                    if (lowPrice === null || highPrice === null) {
                      return true;
                    }

                    return postPrice !== null && postPrice >= lowPrice && postPrice <= highPrice;
                  })
                      ...
                      {index < posts.length - 5 && (
                        <div className={scss.card}>
                          <div className={scss.top_card}>
                            <p>{e.discount} <span><img src={e.status} alt="" /></span></p>
                          ...
                          <div className={scss.middle_card}>
                            <p className={scss.phone_quantity}>В наличии {e.quantity}</p>
                            <h3>{e.name}</h3>
                          ...
                          <div className={scss.bottom_card}>
                            <div className={scss.phone_prices}>
                              <p className={scss.phone_price}>{e.price
                              }</p>
                              <p className={scss.phone_old_price}>{e.oldPrice}</p>
                            </div>
              ...
              <div className={scss.showButtons}>
                {!allPhones && (
                  <button onClick={handleShowAllPhones} className={scss.moreButton}>Показать ещё</button>
                )}
                {allPhonesHide && (
                  <button onClick={handleHideAllPhones} className={scss.moreButton}>Скрыть</button>
                )}
              </div>
            </div>
          </div>
          ...
          <div className={scss.divBottom}>
            ...
            <div className={scss.bottomCards}>
              {posts && posts
                .slice(Math.max(posts.length - 5, 0))
                .map((e, index) => (
                  <div className={scss.cards} key={index}>
