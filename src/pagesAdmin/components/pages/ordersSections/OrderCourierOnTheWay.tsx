...
import { useDeleteAdminOrderMutation, useGetAdminOrderQuery } from "@/src/redux/api/adminOrders"
...
const OrderCourierOnTheWay = () => {
  ...
  const processingOrders = adminOrders?.filter(order => order.status === 'Курьер в пути') || [];
  ...
  const countOrdersByStatus = (orders: any[]) => {
    return orders.reduce((acc: { [x: string]: any }, order: { status: string | number }) => {
            ...
            <div className={scss.content_left_1}>
              ...
              <div className={scss.navigation_div}>
                <Link to={"/admin/orders/in-pending"}>
                  <h3>В ожидании{statusCounts['В ожидании'] > 0 ? ` (${statusCounts['В ожидании']})` : ''}</h3>
                </Link>
                <Link to={"/admin/orders/in-processing"}>
                  <h3>В обработке{statusCounts['В обработке'] > 0 ? ` (${statusCounts['В обработке']})` : ''}</h3>
                </Link>
                <Link to={""}>
                  <h3 className={scss.active_link}>Курьер в пути{statusCounts['Курьер в пути'] > 0 ? ` (${statusCounts['Курьер в пути']})` : ''}</h3>
                </Link>
                <Link to={"/admin/orders/delivered"}>
                  <h3>Доставлены{statusCounts['Доставлены'] > 0 ? ` (${statusCounts['Доставлены']})` : ''}</h3>
                </Link>
                <Link to={"/admin/orders/canceled"}>
                  <h3>Отменены{statusCounts['Отменены'] > 0 ? ` (${statusCounts['Отменены']})` : ''}</h3>
                </Link>
              </div>
                  ...
                  <tr className={scss.tr}>
                      ...
                      <Link to={`single-order/${e._id}`}>
                        <div className={scss.tr_div}>
                          ...
                          <div className={scss.tr_row_2}>
                            <td className={scss.number_col}><h2>{e.number}</h2> <span>{e.date}</span></td>
                            <td className={scss.quantity_col}>{e.quantity}</td>
                            <td className={scss.total_price_col}>{e.totalPrice}</td>
                            <td className={scss.order_type_col}>{e.orderType}</td>
                            <CustomSelect
                              orderId={e._id}
                    ...
                    <ModalWindow open={modalIsOpen}>
                      <div className={scss.modal}>
                        <h2>Вы уверены, что хотите удалить товар
                          <span> {modalName}</span>?</h2>

                        <div className={scss.modal_buttons}>
                          <button onClick={() => setModalIsOpen(false)} className={scss.cancel_modal_button}>Отменить</button>
                          <button onClick={handleDeleteOrder} className={scss.delete_modal_button}>Удалить</button>
                        </div>
                      </div>
                    </ModalWindow>
                  </tr>
